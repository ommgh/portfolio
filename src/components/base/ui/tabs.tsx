...
function TabsList({ className, ...props }: TabsPrimitive.List.Props) {
  return (
    <TabsPrimitive.List
      data-slot="tabs-list"
      className={cn(
        "relative z-0 inline-flex h-8 w-fit items-center justify-center rounded-lg p-0.5",
...
function TabsIndicator({ className, ...props }: TabsPrimitive.Indicator.Props) {
  return (
    <TabsPrimitive.Indicator
      ...
      className={cn(
        "absolute top-0.5 bottom-0.5 left-0 -z-1 w-(--active-tab-width) translate-x-(--active-tab-left) rounded-md bg-white shadow-sm transition-all duration-200 ease-in-out dark:bg-zinc-700",
        className
      )}
      {...props}
    />
