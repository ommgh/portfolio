...
const components: MDXComponents = {
  ...
  blockquote: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <blockquote
      className={cn(
        "mt-6 border-gray-4 border-l-2 pl-6 text-muted-foreground",
        className
      )}
      {...props}
    />
  ...
  ol: ({ className, ...props }: React.HTMLAttributes<HTMLOListElement>) => {
    if (
      ...
      return (
        <ol data-footnotes>
          <div className="mt-6 mb-2 text-muted-foreground text-small">
            Footnotes
          </div>
          {props.children}
        </ol>
  ...
  Figure: ({ children, withCodeblock }) => (
    <Figure withCodeblock={withCodeblock ? withCodeblock : undefined}>
      {children}
    </Figure>
  ),
  ...
  Image: ({ caption, alt, ...props }) => (
    <CustomImage {...props} caption={caption} alt={alt} />
  ),
};
